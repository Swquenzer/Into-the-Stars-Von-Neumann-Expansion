...
import { Probe, SolarSystem, ResourceType, ProbeState, ProbeBlueprint, ProbeStats } from '../types';
import { TURN_COST_PER_DEGREE, UPGRADE_COSTS } from '../constants';
import { 
  Rocket, Pickaxe, Hammer, Radar, BrainCircuit, Pause, 
  MapPin, Satellite, Pencil, Check, X, Radiation, Zap, 
  Trash2, Compass, Navigation, Wind, Skull, Plus, AlertCircle, Clock, Activity, Power
} from 'lucide-react';
...
export const ProbesListPanel: React.FC<ProbesPanelProps> = ({
  ...
  const currentLocation = selectedProbe && selectedProbe.locationId 
    ? systems.find(s => s.id === selectedProbe.locationId) 
    : null;

  const isAnalyzed = currentLocation?.analyzed || false;
  const canMineMetal = currentLocation && currentLocation.resourceYield.Metal > 0;
  const canMinePlutonium = currentLocation && currentLocation.resourceYield.Plutonium > 0;

  // Calculate Turn Cost
  const turnCost = useMemo(() => {
    if (!selectedProbe || selectedProbe.state !== ProbeState.Exploring) return 0;
  ...
  const canAffordTurn = selectedProbe ? selectedProbe.inventory.Plutonium >= turnCost : false;
  const isSolarSailing = selectedProbe?.isSolarSailing || (selectedProbe?.state === ProbeState.Exploring && selectedProbe?.inventory.Plutonium <= 0);

  // Helper to render upgrade row
  const renderUpgradeRow = (statKey: keyof ProbeStats, label: string, icon: React.ReactNode) => {
      ...
      const canAfford = selectedProbe.inventory.Metal >= metalCost && selectedProbe.inventory.Plutonium >= plutoniumCost;
          ...
          <div className="bg-slate-900 p-2 rounded border border-slate-800 flex justify-between items-center group">
              <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-slate-500">
                      Lvl {levelFactor} → <span className="text-cyan-400">Lvl {levelFactor + 1}</span>
                  </div>
              </div>
              <button
    ...
    <div className="flex flex-col h-full bg-slate-900 overflow-hidden w-full">
      ...
      <div className="h-1/3 flex-none overflow-y-auto p-2 space-y-1 min-h-0 border-b border-slate-800">
        {probes.map(probe => (
          ...
          >
            <div>
              <div className={`font-mono text-xs font-bold ${selectedProbeId === probe.id ? 'text-cyan-200' : 'text-slate-400'}`}>
                {probe.name}
              </div>
              <div className="text-[10px] text-slate-500 flex items-center gap-1">
                 {(probe.state === ProbeState.MiningMetal || probe.state === ProbeState.MiningPlutonium) ? (
                    probe.state === ProbeState.MiningMetal ? <Pickaxe size={10} className="text-slate-300" /> : <Radiation size={10} className="text-red-400" />
                  ) : 
                  probe.state === ProbeState.Traveling ? <Rocket size={10} /> : 
                  probe.state === ProbeState.Scanning ? <Radar size={10} className="text-indigo-400" /> :
                  probe.state === ProbeState.Exploring ? <Compass size={10} className="text-emerald-400" /> :
                  probe.state === ProbeState.Replicating ? <Hammer size={10} /> : <Zap size={10} />}
                 {probe.state}
                 {probe.stats.autonomyLevel > 0 && (
                     <span className={`ml-1 font-bold ${probe.isAutonomyEnabled ? 'text-purple-400' : 'text-slate-600'}`}>AI</span>
                 )}
              </div>
            </div>
            {probe.locationId && (
              <div className="text-[10px] text-slate-600 bg-slate-900 px-1.5 py-0.5 rounded">
                {systems.find(s => s.id === probe.locationId)?.name || 'Deep Space'}
              </div>
            )}
          </button>
        ))}
              ...
              <div className="flex-1 mr-2">
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <button onClick={saveName} className="p-1 hover:bg-emerald-900/50 text-emerald-400 rounded"><Check size={14}/></button>
                    <button onClick={() => setIsEditing(false)} className="p-1 hover:bg-red-900/50 text-red-400 rounded"><X size={14}/></button>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 group">
                    <div className="text-sm font-bold text-slate-200">{selectedProbe.name}</div>
                    <button onClick={startEditing} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-cyan-400 transition-opacity">
                      <Pencil size={12} />
                    </button>
                  </div>
                )}
                <div className="text-xs text-slate-500">{selectedProbe.model}</div>
              </div>
            ...
            <div className="flex rounded bg-slate-900 p-0.5 mb-3 flex-none">
                <button 
                    ...
                    className={`flex-1 text-[10px] font-bold py-1 rounded transition-colors ${
                        viewMode === 'status' ? 'bg-slate-700 text-cyan-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'
                    }`}
                >
                    STATUS
                </button>
                <button 
                     ...
                     className={`flex-1 text-[10px] font-bold py-1 rounded transition-colors ${
                        viewMode === 'upgrades' ? 'bg-slate-700 text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'
                    }`}
                >
                    UPGRADE BAY
                </button>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-900 p-2 rounded border border-slate-800">
                            ...
                            <div className="text-slate-300 font-mono text-sm">{Math.floor(selectedProbe.inventory.Metal)}</div>
                        </div>
                        <div className="bg-slate-900 p-2 rounded border border-slate-800">
                            <div className="text-[10px] text-slate-500">PLUTONIUM</div>
                            <div className={`text-sm font-mono ${selectedProbe.inventory.Plutonium < 20 ? 'text-red-400' : 'text-slate-300'}`}>
                    ...
                    {selectedProbe.stats.autonomyLevel > 0 && (
                        <div className="bg-purple-900/20 px-2 py-1.5 rounded border border-purple-800/50 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-bold text-purple-300">
                                        {selectedProbe.stats.autonomyLevel === 1 ? 'ADVENTURER AI' : 'REPLICATOR AI'}
                                    </span>
                                    <span className="text-[9px] text-purple-400/70">Autonomous Logic Installed</span>
                                </div>
                            </div>
                             <button
                            ...
                            >
                                <Power size={10} /> {selectedProbe.isAutonomyEnabled ? 'ON' : 'OFF'}
                            </button>
                        </div>
                    )}
                    ...
                    {/* Performance Stats */}
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-900 px-2 py-1.5 rounded border border-slate-800 flex justify-between items-center" title="Mining Speed Multiplier">
                            <span className="text-[10px] text-slate-500 flex items-center gap-1"><Pickaxe size={10} className="text-yellow-400"/> MINING</span>
                            <span className="text-xs font-mono text-slate-300">{selectedProbe.stats.miningSpeed}x</span>
                        </div>
                        <div className="bg-slate-900 px-2 py-1.5 rounded border border-slate-800 flex justify-between items-center" title="Engine Thrust Multiplier">
                            <span className="text-[10px] text-slate-500 flex items-center gap-1"><Rocket size={10} className="text-red-400"/> THRUST</span>
                            <span className="text-xs font-mono text-slate-300">{selectedProbe.stats.flightSpeed}x</span>
                        </div>
                    </div>

                    {/* Sensor Stats */}
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-900 px-2 py-1.5 rounded border border-slate-800 flex justify-between items-center" title="Sensor Range (Light Years)">
                            <span className="text-[10px] text-slate-500 flex items-center gap-1"><Radar size={10} className="text-indigo-400"/> RANGE</span>
                            <span className="text-xs font-mono text-slate-300">{selectedProbe.stats.scanRange} LY</span>
                        </div>
                        <div className="bg-slate-900 px-2 py-1.5 rounded border border-slate-800 flex justify-between items-center" title="Scan Speed Multiplier">
                            <span className="text-[10px] text-slate-500 flex items-center gap-1"><Activity size={10} className="text-indigo-400"/> SCAN SPD</span>
                            <span className="text-xs font-mono text-slate-300">{selectedProbe.stats.scanSpeed}x</span>
                        </div>
                    </div>
                    ...
                    {selectedProbe.state === ProbeState.Exploring && (
                        <div className="mt-1 flex flex-col gap-0.5">
                            <div className="flex items-center gap-2 text-[10px] text-emerald-400">
                                <Compass size={10} /> Heading: {selectedProbe.heading?.toFixed(0)}°
                            </div>
                            {isSolarSailing && (
                     ...
                     {selectedProbe.state === ProbeState.Traveling && isSolarSailing && (
                        <div className="mt-1 flex items-center gap-2 text-[10px] text-amber-400 animate-pulse">
                             <Wind size={10} /> SOLAR SAILING (5% PWR)
                        </div>
                    )}
                     {selectedProbe.progress > 0 && selectedProbe.state !== ProbeState.MiningMetal && selectedProbe.state !== ProbeState.MiningPlutonium && selectedProbe.state !== ProbeState.Exploring && (
                    <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden">
                        <div 
                        className={`h-full transition-all duration-200 ${selectedProbe.state === ProbeState.Scanning ? 'bg-indigo-500' : 'bg-cyan-500'}`} 
                        style={{ width: `${selectedProbe.progress}%` }}
                        />
                    </div>
                    )}
                    {(selectedProbe.state === ProbeState.MiningMetal || selectedProbe.state === ProbeState.MiningPlutonium) && (
                    <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden">
                        <div 
                        className={`h-full ${selectedProbe.state === ProbeState.MiningPlutonium ? 'bg-red-500' : 'bg-slate-300'}`}
                        style={{ width: `${selectedProbe.progress}%` }}
                        />
                    </div>
                    )}
                    ...
                    <div className="text-[10px] text-slate-600 font-bold mb-2">PROBE ACTIONS</div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        ...
                        <div className="grid grid-cols-3 gap-1">
                            <button
                                ...
                                disabled={selectedProbe.state !== ProbeState.Idle || !selectedProbe.locationId || !canMineMetal || !isAnalyzed}
                                ...
                                title={!isAnalyzed ? "Analysis required to identify resources" : !canMineMetal ? "System depleted of Metal" : ""}
                                >
                                <Pickaxe size={14} /> MINE M.
                                </button>
                                <button
                                onClick={() => onMine(ResourceType.Plutonium)}
                                disabled={selectedProbe.state !== ProbeState.Idle || !selectedProbe.locationId || !canMinePlutonium || !isAnalyzed}
                                ...
                                title={!isAnalyzed ? "Analysis required to identify resources" : !canMinePlutonium ? "System depleted of Plutonium" : ""}
                                >
                            ...
                            <div className="bg-slate-900 p-2 rounded border border-slate-700">
                                <div className="flex justify-between items-center mb-2">
                                    <h4 className="text-[10px] font-bold text-emerald-400">VECTOR CONFIGURATION</h4>
                                    <button onClick={() => setShowDeepSpaceUI(false)} className="text-slate-500 hover:text-white"><X size={12}/></button>
                                </div>
                                <div className="flex items-center gap-2 mb-2">
                                    <div className="relative w-8 h-8 rounded-full border border-slate-600 flex items-center justify-center bg-slate-800">
                                        <Navigation size={12} className="text-emerald-400 transition-transform" style={{ transform: `rotate(${heading + 45}deg)` }} />
                                    </div>
                                    <input 
                                        type="range" 
                                        min="0" 
                                    ...
                                    />
                                    <div className="w-8 text-right font-mono text-xs text-emerald-300">{heading}°</div>
                                </div>
                                ...
                                <button
                                    ...
                                    className={`w-full text-white text-xs font-bold py-1 rounded disabled:opacity-50 disabled:cursor-not-allowed ${
                                        selectedProbe.state === ProbeState.Exploring && !canAffordTurn
                                        ? 'bg-amber-800 hover:bg-amber-700 border border-amber-600'
                                        : selectedProbe.state === ProbeState.Idle && selectedProbe.inventory.Plutonium <= 0
                                        ? 'bg-amber-700 hover:bg-amber-600'
                                        : 'bg-emerald-700 hover:bg-emerald-600'
                                    }`}
                                >
                                    {selectedProbe.state === ProbeState.Exploring 
                                      ? (canAffordTurn ? 'ADJUST COURSE' : 'EMERGENCY TURN (DRAIN FUEL)')
                                      : (selectedProbe.inventory.Plutonium > 0 ? 'ENGAGE THRUSTERS' : 'ENGAGE SOLAR SAILS (SLOW)')
                                    }
                                </button>
                            </div>
                        )}
                        ...
                        {!showDeepSpaceUI && (
                            <div className="grid grid-cols-2 gap-1">
                                <button
                                    ...
                                    disabled={selectedProbe.state !== ProbeState.Idle && selectedProbe.state !== ProbeState.Exploring}
                                ...
                                >
                                    <Compass size={12} /> {selectedProbe.state === ProbeState.Exploring ? 'ADJUST COURSE' : 'DEEP SPACE'}
                                </button>
                        ...
                        {(selectedProbe.state !== ProbeState.Idle && selectedProbe.state !== ProbeState.Traveling && selectedProbe.state !== ProbeState.Exploring) && (
                            <button
                        ...
                        <div className="mt-1">
                            <div className="flex justify-between items-end mb-1">
                                <div className="text-[10px] text-slate-600 font-bold">CREATE NEW PROBE</div>
                                <button 
                            ...
                            {blueprints.map((bp) => {
                                const cost = bp.cost;
                                const canAfford = selectedProbe.inventory.Metal >= cost.Metal && selectedProbe.inventory.Plutonium >= cost.Plutonium;
                                const effectiveTime = Math.ceil(cost.time / selectedProbe.stats.replicationSpeed / 1000);
                                
                                return (
                                    <button
                                    key={bp.id}
                                    onClick={() => onReplicate(bp)}
                                    disabled={selectedProbe.state !== ProbeState.Idle || !canAfford}
                                    ...
                                    >
                                    <div className="font-bold truncate pr-3">{bp.name}</div>
                                    <div className="opacity-60 flex justify-between items-center">
                                        <span>M:{cost.Metal} P:{cost.Plutonium}</span>
                                        <span className="text-[9px] flex items-center gap-0.5"><Clock size={9}/> {effectiveTime}s</span>
                                    </div>
                                    {bp.isCustom && (
                                        <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover/bp:opacity-100">
                                        <div 
                                            onClick={(e) => { e.stopPropagation(); onOpenDesigner(bp); }}
            ...
            {viewMode === 'upgrades' && (
                <div className="space-y-2 animate-in fade-in duration-300">
                     <div className="text-[10px] text-slate-500 mb-2 italic bg-slate-900/50 p-2 rounded">
                        Consume local resources to enhance probe capabilities. Probe must be Idle.
                     </div>
                     ...
                     <div className="flex justify-between items-center bg-slate-950 p-2 rounded border border-slate-800 mb-2">
                         <div className="text-xs text-slate-300 font-mono">M: {Math.floor(selectedProbe.inventory.Metal)}</div>
                         <div className="text-xs text-slate-300 font-mono">P: {Math.floor(selectedProbe.inventory.Plutonium)}</div>
                     </div>

                     {renderUpgradeRow('miningSpeed', 'Mining Speed', <Pickaxe size={12} className="text-yellow-400" />)}
                     {renderUpgradeRow('flightSpeed', 'Flight Speed', <Rocket size={12} className="text-red-400" />)}
                     {renderUpgradeRow('scanRange', 'Scan Range', <Radar size={12} className="text-indigo-400" />)}
                     {renderUpgradeRow('scanSpeed', 'Scan Processing', <Activity size={12} className="text-blue-400" />)}
                     {renderUpgradeRow('replicationSpeed', 'Fabrication', <Hammer size={12} className="text-emerald-400" />)}
                     {renderUpgradeRow('autonomyLevel', 'Neural Core', <BrainCircuit size={12} className="text-purple-400" />)}
                </div>
            )}
