...
import { 
  Activity, Rocket, Pickaxe, Radiation, Hammer, Radar, Compass, Wind
} from 'lucide-react';
...
export const OperationsListPanel: React.FC<OperationsPanelProps> = ({ probes, systems, onProbeSelect }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900 overflow-hidden w-full">
            <div className="p-3 bg-slate-800/50 border-b border-slate-700 flex justify-between items-center flex-none">
                ...
                <span className="text-xs text-slate-500">{activeProbes.length} Active</span>
            </div>
            <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-0">
                {activeProbes.length === 0 && (
                    <div className="text-slate-600 text-xs italic text-center mt-4">No active operations</div>
                )}
                {activeProbes.map(probe => {
                    ...
                    if (probe.state === ProbeState.Traveling && probe.targetSystemId) {
                        targetName = systems.find(s => s.id === probe.targetSystemId)?.name || "Unknown";
                        details = probe.isSolarSailing ? ` solar sailing to ${targetName}` : ` traveling to ${targetName}`;
                        progressColor = probe.isSolarSailing ? "bg-amber-500" : "bg-red-500";
                        OperationIcon = probe.isSolarSailing ? Wind : Rocket;
                    } else if (probe.state === ProbeState.MiningMetal && probe.locationId) {
                        const locName = systems.find(s => s.id === probe.locationId)?.name || "Unknown";
                        details = ` mining Metal at ${locName}`;
                        if (probe.stats.autonomyLevel > 0) details = " (Auto) " + details;
                        progressColor = "bg-slate-300";
                        OperationIcon = Pickaxe;
                    } else if (probe.state === ProbeState.MiningPlutonium && probe.locationId) {
                        const locName = systems.find(s => s.id === probe.locationId)?.name || "Unknown";
                        ...
                        if (probe.pendingBlueprint) {
                            const totalTimeMs = probe.pendingBlueprint.cost.time / Math.max(0.1, probe.stats.replicationSpeed);
                        ...
                        details = sailing ? ` drifting (Head: ${probe.heading?.toFixed(0)}°)` : ` exploring (Head: ${probe.heading?.toFixed(0)}°)`;
                         if (probe.stats.autonomyLevel > 0) details = " (Auto) " + details;
                        progressColor = sailing ? "bg-amber-500" : "bg-emerald-500";
                        OperationIcon = sailing ? Wind : Compass;
                    }

                    const displayPct = probe.state === ProbeState.Traveling ? Math.floor(probe.progress * 100) : Math.floor(probe.progress);
                    ...
                    return (
                        ...
                        >
                            <div className="flex justify-between items-start mb-1">
                                <div className="font-bold text-xs text-slate-300">{probe.name}</div>
                            ...
                            {probe.state !== ProbeState.Exploring ? (
                              <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden">
                                  <div className={`${progressColor} h-full`} style={{ width: `${displayPct}%` }} />
                              </div>
                            ) : (
                              <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden flex">
                                 {/* Pulse bar for exploring */}
                                  <div className={`h-full w-full ${probe.inventory.Plutonium <= 0 ? 'bg-amber-500/20' : 'bg-emerald-500/20'} animate-pulse`} />
                              </div>
                            )}
                        </button>
                    );
