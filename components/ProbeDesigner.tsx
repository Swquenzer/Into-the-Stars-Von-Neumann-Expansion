...
import { X, Save, Cpu, Zap, Activity, Radar, Rocket, BrainCircuit } from 'lucide-react';
...
export const ProbeDesigner: React.FC<ProbeDesignerProps> = ({ initialBlueprint, onClose, onSave }) => {
  const [name, setName] = useState(initialBlueprint?.name || 'New Design');
  const [stats, setStats] = useState<ProbeStats>(initialBlueprint?.stats || {
    miningSpeed: 0,
  ...
  useEffect(() => {
    ...
    plutonium += stats.replicationSpeed * COST_MULTIPLIERS.replicationSpeed.Plutonium;
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Core Specifications</h3>
            ...
            <div className="pt-4 border-t border-slate-800 space-y-6">
              <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Sensors & AI</h3>
              ...
              <div>
                <div className="flex justify-between mb-2">
                    <div className="flex items-center gap-2 text-slate-200 font-bold text-sm">
                        <BrainCircuit size={16} className="text-purple-400"/> AI Core System
                    </div>
                </div>
                ...
                <div className="text-[10px] text-slate-500 mt-1 h-8">
                    {stats.autonomyLevel === 0 && "Probe requires direct manual control for all operations."}
                    {stats.autonomyLevel === 1 && "Autonomous exploration and scanning. Will mine fuel as needed."}
                    {stats.autonomyLevel === 2 && "Full autonomy. Will explore, mine resources, and replicate itself automatically."}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-950 rounded-xl p-6 border border-slate-800 flex flex-col justify-between">
             <div>
                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-6">Blueprint Analysis</h3>
                
                <div className="space-y-4">
                  <CostRow label="Metal Cost" value={cost.Metal} color="text-slate-200" />
                  <CostRow label="Plutonium Cost" value={cost.Plutonium} color="text-red-400" />
                  <div className="h-px bg-slate-800 my-4" />
                  <CostRow label="Fabrication Time" value={`${(cost.time / 1000).toFixed(1)}s`} color="text-cyan-400" />
                </div>
             </div>
             ...
             <div className="mt-8 bg-slate-900 p-4 rounded text-xs text-slate-400 border border-slate-800">
                ...
                <ul className="list-disc pl-4 space-y-1">
                   {stats.flightSpeed > 7 && <li className="text-red-300">High-thrust engines require significant plutonium.</li>}
                   {stats.replicationSpeed > 3 && <li className="text-emerald-300">Advanced fabrication modules drastically increase complexity.</li>}
                   {cost.Metal > 1000 && <li className="text-yellow-300">Warning: High metal cost may exceed standard storage limits of smaller probes.</li>}
                   {stats.scanRange > 600 && <li className="text-indigo-300">Long-range sensors enabled.</li>}
                   {stats.scanSpeed === 1 && <li className="text-blue-300">Basic sensor processing included at no extra cost.</li>}
                   {stats.autonomyLevel > 0 && <li className="text-purple-300 font-bold">Neural Net AI Core Installed. High resource cost.</li>}
                </ul>
             </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 flex justify-end gap-4">
          ...
          <button 
            onClick={handleSave}
            disabled={cost.Metal === BASE_COST.Metal && cost.Plutonium === BASE_COST.Plutonium}
          ...
          >
            <Save size={18} /> {initialBlueprint ? 'Update Blueprint' : 'Save Blueprint'}
          </button>
        </div>
      </div>
    </div>
...
const StatSlider: React.FC<{ 
  ...
  <div>
    <div className="flex justify-between mb-1">
      ...
      <div className="text-cyan-400 font-mono font-bold">{value}{unit}</div>
    </div>
    <input 
      type="range" 
      min="0" 
...
const CostRow: React.FC<{ label: string; value: string | number; color: string }> = ({ label, value, color }) => (
