...
import { 
  Radio, AlertCircle, LocateFixed, BrainCircuit, Send, Wind, Compass
} from 'lucide-react';
...
export const SystemsListPanel: React.FC<SystemsPanelProps> = ({
  ...
  const fuelCost = selectedProbe && selectedSystem 
      ? Math.floor(getDistance(selectedSystem, selectedProbe) * FUEL_CONSUMPTION_RATE) 
      : 0;
  const canAffordLaunch = selectedProbe ? selectedProbe.inventory.Plutonium >= fuelCost : false;

  const isProbePresent = selectedSystem 
    ? probes.some(p => p.locationId === selectedSystem.id && p.state !== ProbeState.Traveling)
    : false;

  // Calculate angle and cost to selected system for Deep Space
  const deepSpaceVector = useMemo(() => {
      if (!selectedProbe || !selectedSystem || selectedProbe.state !== ProbeState.Exploring) return null;
  ...
  const visibleSystems = useMemo(() => {
    return systems.filter(s => s.discovered || s.visited).sort((a, b) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 overflow-hidden w-full">
       <div className="p-3 bg-slate-800/50 border-b border-slate-700 flex justify-between items-center flex-none">
        ...
        <span className="text-xs text-slate-500">{systems.filter(s => s.visited || s.analyzed).length} / {systems.length}</span>
      </div>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                   <div className={`font-mono text-xs font-bold truncate ${isSelected ? 'text-amber-200' : 'text-slate-400'}`}>
                    {system.name}
                  </div>
                  {!isKnown && (
              ...
              {isKnown && (
                <div className="flex gap-1 flex-none ml-2">
                   <div className="w-1.5 h-6 bg-slate-800 rounded-sm overflow-hidden flex flex-col justify-end">
                      <div className="bg-slate-400 w-full" style={{ height: `${system.resources.Metal}%` }} title="Metal" />
                   </div>
                   <div className="w-1.5 h-6 bg-slate-800 rounded-sm overflow-hidden flex flex-col justify-end">
                      <div className="bg-red-400 w-full" style={{ height: `${system.resources.Plutonium}%` }} title="Plutonium" />
                   </div>
                </div>
              )}
            </button>
      ...
      {selectedSystem && (
         <div className="flex-1 p-3 bg-slate-950/50 overflow-y-auto min-h-0">
            ...
            <div className="space-y-3">
               <div className="text-sm font-bold text-slate-200">{selectedSystem.name}</div>
               
               <div className="bg-slate-900 p-2 rounded border border-slate-800">
                {selectedSystem.lore ? (
                  <p className="text-slate-400 italic text-xs leading-relaxed">"{selectedSystem.lore}"</p>
                ) : (
                  <button 
                  ...
                  >
                    <BrainCircuit size={12} /> {isProbePresent ? 'Analyze Composition' : 'Probe Required'}
                  </button>
                )}
              </div>
               ...
               {selectedSystem.analyzed ? (
                 <div className="space-y-1">
                    ...
                    <div className="w-full bg-slate-900 h-1 rounded-full overflow-hidden">
                       <div className="bg-slate-400 h-full" style={{ width: `${selectedSystem.resources.Metal}%` }} />
                    </div>
                    <div className="flex justify-between text-[10px] text-slate-400 mt-1">
                     ...
                     <div className="w-full bg-slate-900 h-1 rounded-full overflow-hidden">
                       <div className="bg-red-400 h-full" style={{ width: `${selectedSystem.resources.Plutonium}%` }} />
                    </div>
                 </div>
               ) : (
               ...
               <div>
                  <div className="text-[10px] text-slate-600 font-bold mb-1">DOCKED PROBES</div>
                  <div className="flex flex-wrap gap-1">
                    {probes.filter(p => p.locationId === selectedSystem.id).length > 0 ? (
                      probes.filter(p => p.locationId === selectedSystem.id).map(p => (
                        <button 
                          key={p.id}
               ...
               <div className="pt-2 border-t border-slate-800 space-y-2">
                 {/* Standard Launch */}
                 {selectedProbe && selectedProbe.locationId !== selectedSystem.id && selectedProbe.state !== ProbeState.Exploring && (
                    <button
                    ...
                    >
                      {selectedProbe.state !== ProbeState.Idle ? <Send size={12} /> : canAffordLaunch ? <Send size={12} /> : <Wind size={12} />}
                 ...
                 {deepSpaceVector && selectedProbe && selectedProbe.state === ProbeState.Exploring && (
                     <button
